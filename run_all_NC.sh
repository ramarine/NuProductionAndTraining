...
FCL_FILE_NAME="prodgenie_atmnNC_max_weighted_dune10kt_1x2x6"
...
job2=$(sbatch $COMMON_SBATCH_OPTS -J NC_g4 --dependency=afterok:$job1 --output=$LOG_DIR/slurm-%x-%j.out --error=$LOG_DIR/slurm-%x-%j.err run_g4.sh $NUM_EVENTS "$DATA_DIR" $FCL_FILE_NAME | awk '{print $4}')
job2_a=$(sbatch $LOG_SBATCH_OPTS -J NC_del_gen_events --dependency=afterok:$job2 --output=$LOG_DIR/slurm-%x-%j.out --error=$LOG_DIR/slurm-%x-%j.err delete_gen_events.sh "$DATA_DIR" | awk '{print $4}')
job3=$(sbatch $DETSIM_SBATCH_OPTS -J NC_detsim --dependency=afterok:$job2_a --output=$LOG_DIR/slurm-%x-%j.out --error=$LOG_DIR/slurm-%x-%j.err run_detsim.sh $NUM_EVENTS "$DATA_DIR" $FCL_FILE_NAME | awk '{print $4}')
job3_a=$(sbatch $LOG_SBATCH_OPTS -J NC_del_g4 --dependency=afterok:$job3 --output=$LOG_DIR/slurm-%x-%j.out --error=$LOG_DIR/slurm-%x-%j.err delete_g4.sh "$DATA_DIR" | awk '{print $4}')
