...
usage() {
  ...
  echo "Example: $0 -p tau -i CC -e 1000 -f 10"
  echo ""
...
elif [ $PARTICLE_TYPE == "tau_test" ]; then
    DATA_DIR="/scratch/amarinei/data/Atmospherics/Tau"${INTERACTION_TYPE}"_${NUM_EVENTS}_${NUM_FILES}"
    # FCL_FILE_NAME="prodgenie_atmnu_max_weighted_dune10kt"
    FCL_FILE_NAME="prodgenie_atmnu_max_weighted_randompolicy_dune10kt"
    # FCL_FILE_NAME="standard_reco2_atmos_dune10kt"
    ARRAY_MIN=1
    ARRAY_MAX=$((ARRAY_MIN + NUM_FILES-1))
...
COMMON_SBATCH_OPTS="--account=def-nilic --time=1:30:00 --mem=${MEMORY}G --mail-user=robert.mihai.amarinei@cern.ch --mail-type=BEGIN,END,FAIL --array=$ARRAY_MIN-$NUM_FILES"
...
job5=$(sbatch $COMMON_SBATCH_OPTS -J ${PARTICLE_TYPE}_makehdf5 --output=$LOG_DIR/slurm-%x-%j.out --error=$LOG_DIR/slurm-%x-%j.err run_makehdf5.sh $NUM_EVENTS "$DATA_DIR" $FCL_FILE_NAME | awk '{print $4}')
