...
LOG_SBATCH_OPTS="--account=def-nilic --time=0:35:00 --mem=1G --mail-user=robert.mihai.amarinei@cern.ch "
COMMON_SBATCH_OPTS="--account=def-nilic --time=15:30:00 --mem=${MEMORY}G --array=$ARRAY_MIN-$ARRAY_MAX"
DETSIM_SBATCH_OPTS="--account=def-nilic --time=1-1:30:00 --mem=${MEMORY}G --array=$ARRAY_MIN-$ARRAY_MAX"
...
job6=$(sbatch $COMMON_SBATCH_OPTS -J ${PARTICLE_TYPE}_makehdf5 --dependency=afterok:$job4 --output=$LOG_DIR/slurm-%x-%j.out --error=$LOG_DIR/slurm-%x-%j.err run_makehdf5.sh $NUM_EVENTS "$DATA_DIR" $FCL_FILE_NAME | awk '{print $4}')
...
job8=$(sbatch $LOG_SBATCH_OPTS --dependency=afterany:$job7 --output=$LOG_DIR/clean_%j.out --error=$LOG_DIR/clean_%j.err --wrap="python3 explore_logs/clean_job_times.py $LOG_DIR job_times.log" | awk '{print $4}')

# sbatch $LOG_SBATCH_OPTS --output=$LOG_DIR/clean_%j.out --error=$LOG_DIR/clean_%j.err --wrap="python3 explore_logs/clean_job_times.py $LOG_DIR/ job_times.log" | awk '{print $4}'
