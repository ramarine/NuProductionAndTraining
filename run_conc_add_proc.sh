...
STEP1_OPTS="--time=1:30:00 --mem=230G --nodes=1 --ntasks=32 $COMMON_OPTS"
# Add key sbatch options
STEP2_OPTS="--time=5:30:00 --mem=20G $COMMON_OPTS"
#  sbatch options
STEP3_OPTS="--time=20:30:00 --mem=430G --nodes=1 --ntasks=32 $COMMON_OPTS"
...
# job2=$(sbatch $STEP2_OPTS --dependency=afterok:$job1 --output=${LOG_DIR}/add_key_%j.out --error=${LOG_DIR}/add_key_%j.err ./run_add_key.sh "$INFILE_DIR" "$OUT_NAME" | awk '{print $4}')
