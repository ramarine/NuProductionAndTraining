...
# Submit job 1
job1=$(sbatch $STEP1_OPTS --output=${LOG_DIR}/combine_hdf5_%j.out --error=${LOG_DIR}/combine_hdf5_%j.err ./combine_hdf5.sh "$INFILE_DIR" "$OUT_NAME" | awk '{print $4}')
...
job4=$(sbatch $STEP3_OPTS --dependency=afterok:$job3 --output=${LOG_DIR}/merge_%j.out --error=${LOG_DIR}/merge_%j.err ./merge.sh "$INFILE_DIR" "$OUT_NAME" | awk '{print $4}')
