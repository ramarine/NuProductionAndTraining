...
NUM_FILES=1000
MEMORY=35
DATA_DIR="/scratch/amarinei/data/Atmospherics/ECC_${NUM_EVENTS}_${NUM_FILES}"
...
job2=$(sbatch $COMMON_SBATCH_OPTS -J e_g4  --output=$LOG_DIR/slurm-%x-%j.out --error=$LOG_DIR/slurm-%x-%j.err run_g4.sh $NUM_EVENTS "$DATA_DIR" $FCL_FILE_NAME | awk '{print $4}')
job2_a=$(sbatch $LOG_SBATCH_OPTS -J e_del_gen_events --dependency=afterok:$job2 --output=$LOG_DIR/slurm-%x-%j.out --error=$LOG_DIR/slurm-%x-%j.err delete_gen_events.sh "$DATA_DIR" | awk '{print $4}')
job3=$(sbatch $DETSIM_SBATCH_OPTS -J e_detsim --dependency=afterok:$job2_a --output=$LOG_DIR/slurm-%x-%j.out --error=$LOG_DIR/slurm-%x-%j.err run_detsim.sh $NUM_EVENTS "$DATA_DIR" $FCL_FILE_NAME | awk '{print $4}')
job3_a=$(sbatch $LOG_SBATCH_OPTS -J e_del_g4 --dependency=afterok:$job3 --output=$LOG_DIR/slurm-%x-%j.out --error=$LOG_DIR/slurm-%x-%j.err delete_g4.sh "$DATA_DIR" | awk '{print $4}')
job4=$(sbatch $COMMON_SBATCH_OPTS -J e_reco --dependency=afterok:$job3_a --output=$LOG_DIR/slurm-%x-%j.out --error=$LOG_DIR/slurm-%x-%j.err run_reco.sh $NUM_EVENTS "$DATA_DIR" $FCL_FILE_NAME | awk '{print $4}')
job4_a=$(sbatch $LOG_SBATCH_OPTS -J e_del_detsim --dependency=afterok:$job4 --output=$LOG_DIR/slurm-%x-%j.out --error=$LOG_DIR/slurm-%x-%j.err delete_detsim.sh "$DATA_DIR" | awk '{print $4}')
job5=$(sbatch $COMMON_SBATCH_OPTS -J e_makehdf5 --dependency=afterok:$job4_a --output=$LOG_DIR/slurm-%x-%j.out --error=$LOG_DIR/slurm-%x-%j.err run_makehdf5.sh $NUM_EVENTS "$DATA_DIR" $FCL_FILE_NAME | awk '{print $4}')
